    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yundongCollection !=null){
            ...
            BeanUtils.copyProperties( yundongCollection , view );//把实体数据重构到view中
            //级联表 运动教程
            //级联表
            YundongEntity yundong = yundongService.selectById(yundongCollection.getYundongId());
            if(yundong != null){
            BeanUtils.copyProperties( yundong , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//            yundongCollection.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<YundongCollectionEntity> yundongCollectionList = new ArrayList<>();//上传的东西
        ...
        YundongCollectionEntity yundongCollection = yundongCollectionService.selectById(id);
            if(yundongCollection !=null){
                ...
                //级联表
                    YundongEntity yundong = yundongService.selectById(yundongCollection.getYundongId());
                if(yundong != null){
                    BeanUtils.copyProperties( yundong , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYundongId(yundong.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(yundongCollection.getYonghuId());
